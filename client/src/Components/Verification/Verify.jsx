...
const Verify = () => {
    ...
    return (
        <Container fluid style={{ height: '100vh' }} className='d-flex justify-content-center align-items-center'>
            <Card style={{ width: '60%', height: '300px', background: '#52AB98', color: '#fff', textAlign: 'center' }}>
                <Card.Body className='mt-5'>
                    <Card.Title>{name}</Card.Title>
                    <Card.Text className='mx-auto p-4 fs-4 fw-bold' style={{ width: '350px', background: '#fff', color: '#52AB98' }}>
                        {isverified ? verified.message : verified.message}
                    </Card.Text>
                    {isverified ? <Link to="/login" className='text-white fs-5' replace>Login</Link> : <div>You cannot login without verify your Email</div>}
                </Card.Body>
            </Card>
        </Container>
    )
